...
runfile_location = os.getenv('HOME')+'/Hermit4/vega/test3'
...
#radius in km unit
    ...
    dens_p = 1.0
    mp=[dens_p*4*math.pi*(i*1e5)**3/(3*m_sun_g) for i in radius] + [m_j]
...
#    ap = 14.
    ...
    aj = 80.
    ej = 0.4
    delt_a = aj * 1.8 * ej**0.2 * (m_j/m_sun)**0.2
    r_in = aj - delt_a
    r_edge = aj + delt_a
    print r_in, r_edge
    while True:
...
#    p=[math.sqrt(a[i]**3)/(1+mp[i]) for i in range(n)]
    ...
    print >>f, g_p, g_d, g_r, t_dep/t_unit, r_edge/a_unit, r_in/a_unit, dens0/(m_unit/a_unit**2), dens_p/(m_unit/a_unit**3), m_crit/m_unit, r_esc/a_unit, kg, ej, cd, r_mhs, three_d, n_min, twogg
