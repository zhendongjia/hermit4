...
data_ab = sorted([(i, counter_ab[i]/sum_weight_ab,
                   counter_ab[i] / sum_weight_ab /
                   math.sqrt(num_ab[bin_id(i, binsize)]))
                  for i in counter_ab])
data_i = sorted([(i, counter_i[i]/sum_weight_i,
...
item_model_ab = Gnuplot.PlotItems.Data(
    data_ab, title='final', with_='yerrorlines lc rgb "green"')
item_obs = Gnuplot.PlotItems.File(
    'r_fobs', using='1:2', title='observation',
    with_='lp lc rgb "black"')

g._add_to_queue([item_model_i, item_model_ab, item_obs])
