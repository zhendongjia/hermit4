...
def get_resonance_location(mp, ap, dens):
    a_re = dens/6e-3
    if dens != 0.0:
        a_re = a_re ** 0.25
    else: a_re = None
...
tend = 5e6
tstep = 1e3
nstep = int(tend / tstep)
data0 = []
for i in range(nstep):
    dens = math.exp(- tstep * i / 0.5e6)
    ar = get_resonance_location(1e-3, 5.2, dens)
    data0.append([i*tstep, ar])
#item0 = Gnuplot.PlotItems.Data(data0, with_='l lt 1 lc rgb "green" lw 5' )
