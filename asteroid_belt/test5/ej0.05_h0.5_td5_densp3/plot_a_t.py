...
def get_resonance_location(dens):
    a_re = dens/6e-3
    if dens != 0.0:
        a_re = a_re ** 0.25
    else: a_re = None
...
item0 = Gnuplot.PlotItems.File('t_assr', title = '', with_='l lt 1 lc rgb "green" lw 10' )
item1 = Gnuplot.PlotItems.File('t_assr_wg', title = '', with_='l lt 1 lc rgb "green" lw 10' )
g._add_to_queue([item0, item1])
