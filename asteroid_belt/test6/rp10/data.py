...
if open_1 > 0:
    ...
    f18 = open('rab','w')
    f20 = open('ri','w')
    f30 = open('ri_20_200','w')
    f31 = open('rab_20_200','w')
    f17 = open('a_e','w')
    f22 = open('aj_ej','w')
...
#    if os.listdir('%s' %loop) == '': continue 
    ...
    if os.path.isfile('a_t'): os.system("rm a_t")
    ...
    for line in open('output').readlines():
	    ...
	    if line.startswith(' ESCAPE    I NAM N ECC R A    3'): 
		print 'Saturn Escape', loop + 1
		break
            finish_test = 1
            time += 100.0
            current_orbit = Orbit(line, time, units)
            ...
            orbit[-1][int(current_orbit.name)-1] = current_orbit
	    break
        if line.startswith('         END RUN'): finish_test = 1
	

    radius_open = 0.0
    for i in orbit[0]:
	if i.mp < 1e-4: radius_open = i.radius
	if radius_open > 1000. or radius_open < 10.:
		finish_test = 0
        if i.mp < 1e-4 and i.a > a_out:
                finish_test = 0

    finish_test = 1
	
    if finish_test == 0: 
	print ' not finished running', loop + 1
	os.chdir('..')
	continue
    ...
    if open_5 > 0:
            ...
            for time_orbit in orbit:
				    ...
				    print >> f16, None, None,
                                   ...
                                   print >> f12, i.a, i.ecc,
                                   print >> f16, i.t_tidal1, i.t_tidal2,
                    l_tot = get_angular_momentum(time_orbit)
                    print >> f3, num,
                    for j in range(n):
                            if time_orbit[j] != None:
                                    print >> f1, time_orbit[j].time/tdep
                                    print >> f2, time_orbit[j].time/tdep
                                    print >> f3, time_orbit[j].time/tdep
                                    print >> f4, time_orbit[j].time/tdep
                                    print >> f5, time_orbit[j].time/tdep
                                    print >> f12, time_orbit[j].time/tdep
                                    print >> f16, time_orbit[j].time/tdep
                                    break
            f1.close()
            f2.close()
            f3.close()
...
#    f8.close()
    ...
    if open_1 > 0:
             ...
             for j in orbit[-1]:
                    if j is None: continue
		   # if j.mp > 1e-4 and j.mp < 5e-4: test_j = 1 
                    if j.a >= a_in and j.a <= a_out and j.mp < 1e-4:
                        print >> f18, j.radius, j.a, j.ecc, loop + 1
			print >> f17, j.a, j.ecc
			if j.radius >= 20. and j.radius <= 200.:
				print >> f31, j.radius          
	    # if test_j == 1: print >> f22, orbit[-1][1].a, orbit[-1][1].ecc, orbit[-1][2].a, orbit[-1][2].ecc
                    if j.mp > 1e-4: test_j = 1
             if test_j == 1:
                     print >> f22, orbit[-1][1].a, orbit[-1][1].ecc, orbit[-1][2].a, orbit[-1][2].ecc
             else:
                     print >> f22, orbit[-2][1].a, orbit[-2][1].ecc, orbit[-2][2].a, orbit[-2][2].ecc
